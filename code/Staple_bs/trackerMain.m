...
%% BACKGROUND SUPPERSION
    discussable = 0;
    bsparams.isable = 1;
%     bsparams.lambda = p.params.La;
%     bsparams.learning_rate = p.params.Rate;
%     bsparams.fuse_factor = p.params.Fu;
%     bsparams.gaussianSigma = p.params.Sigma;
%     bsparams.Sw_Size = 0.7;
    bsparams.lambda = 1;
    bsparams.learning_rate = 0.3;
    bsparams.fuse_factor = 0.2;
    bsparams.Sw_Size = 0.7;
    bsparams.gaussianSigma = 0.3;
%% INITIALIZATION
    ...
    for frame = 1:num_frames
	    ...
	    %% TESTING step
            ...
            if discussable,
                ...
                gt = dlmread(['../../anno/skating1.txt']);
                gtpos(1) = gt(frame,2)+floor(gt(frame,4)/2);
                gtpos(2) = gt(frame,1)+floor(gt(frame,3)/2);
                rect_size = p.norm_bg_area;
                input = gtpos-pos+rect_size/2;
                gtgaussian = gaussianBStest(input, rect_size, 0.3);
                if size(im_patch_bg,3)==3,
                    for channel=1:3
                        gtbspatch(:,:,channel) = im2double(im_patch_cf(:,:,channel)).*gtgaussian;
                    end
                else
                    gtbspatch = im2double(im_patch_cf).*gtgaussian;
                end
                gtfeatures = getFeatureMap(gtbspatch, p.feature_type, p.cf_response_size, p.hog_cell_size);
                %             temp1 = abs(gtfeatures-xt);
                temp1 = bsxfun(@times,gtfeatures-xt,gtfeatures-xt);
                disP2gtbs = sum(sum(sum(temp1)));
            ...
            % bs testing
            if bsparams.isable,
                % bshf = bsxfun(@rdivide, bsxtf_num, sum(bsxtf_den, 3) + bsparams.lambda);
                bs_xtf = backSupper(xtf,bsxtf_num,bsxtf_den,bsparams.lambda);
                xtf = bs_xtf * bsparams.fuse_factor + xtf * (1 - bsparams.fuse_factor);   
            end
            
            if discussable,
                %            -----GTbs_comparestart----
                bs_xtff = backSupper(xtf,bsxtf_num,bsxtf_den,bsparams.lambda);
                bs_xt = ensure_real(ifft2(bs_xtff));
                %             bs_xt = ifftshift(bs_xt);
                %             temp2 = abs(gtfeatures-bs_xt);
                temp2 = bsxfun(@times,gtfeatures-bs_xt,gtfeatures-bs_xt);
                disBP2gtbs = sum(sum(sum(temp2)));
                %            -----GTbs_compareend----
            end
        ...
        % bs add
        if bsparams.isable,
            %im_patch_bg_gray = rgb2gray(im_patch_bg);
            patch_size = size(im_patch_bg(:,:,1));
%             bsparams.squareWave = squareWave(patch_size(1),patch_size(2),bsparams.Sw_Size);
            bsparams.gaussian = gaussianForBS(patch_size, bsparams.gaussianSigma);
            if size(im_patch_bg,3)==3,
                for channel=1:3
                    bspatch(:,:,channel) = im2double(im_patch_bg(:,:,channel)) .* bsparams.gaussian;
                end
            else
                bspatch = im2double(im_patch_bg) .* bsparams.gaussian;
            end

            bsxt = getFeatureMap(bspatch, p.feature_type, p.cf_response_size, p.hog_cell_size);
            bsxtf = fft2(bsxt);
        end
        ...
        if bsparams.isable,
            new_bsxtf_num = bsxfun(@times, conj(bsxtf), xtf) / prod(p.cf_response_size);
            new_bsxtf_den = (conj(xtf) .* xtf) / prod(p.cf_response_size);
        end
        if frame == 1
            ...
            % bs add
            if bsparams.isable,
                bsxtf_num = new_bsxtf_num;
                bsxtf_den = new_bsxtf_den;
            end
		else
            ...
            if bsparams.isable,
                bsxtf_den = (1 - bsparams.learning_rate) * bsxtf_den + bsparams.learning_rate * new_bsxtf_den;
                bsxtf_num = (1 - bsparams.learning_rate) * bsxtf_num + bsparams.learning_rate * new_bsxtf_num;
            end
            %% BG/FG MODEL UPDATE
            % patch of the target + padding
